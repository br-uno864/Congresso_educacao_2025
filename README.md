# Desigualdades Regionais e Étnico-Raciais na Conclusão do Ensino no Brasil (2012–2022)

**Congresso Nacional – Educação, Inovação e Tecnologia (2025)**

Este repositório apresenta uma análise das taxas de conclusão do ensino fundamental, médio e superior no Brasil, no período de 2012 a 2022, com foco nas desigualdades regionais e étnico-raciais, a partir de dados oficiais da PNAD Contínua (IBGE).

---

## 📌 Objetivo do Projeto

Investigar como as taxas de conclusão escolar variam:

- Entre as regiões brasileiras, com ênfase comparativa entre Sudeste e Nordeste;
- Entre os grupos de raça/cor, conforme classificação adotada pela PNAD Contínua;
- Ao longo do tempo (2012–2022);
- Entre os diferentes níveis de ensino: fundamental, médio e superior.

O estudo busca evidenciar desigualdades estruturais persistentes no acesso e na permanência educacional no Brasil.

---

## 📊 Fontes de Dados

Os dados utilizados foram extraídos das tabelas da PNAD Contínua – IBGE:

- Tabela 2.5 – Conclusão dos anos iniciais do ensino fundamental  
- Tabela 2.5.1 – Conclusão do ensino fundamental  
- Tabela 2.6.2 – Conclusão do ensino médio  
- Tabela 2.6.3 – Conclusão do ensino superior  

---

## 📍 Escopo da Análise

- Comparação regional entre Sudeste e Nordeste;
- Evolução das taxas de conclusão escolar no período 2012–2022;
- Análise por nível de ensino (fundamental, médio e superior);
- Recortes por raça/cor;
- Identificação de padrões e disparidades regionais e raciais.

---

## 📌 Principais Resultados

- A região Sudeste apresenta, de forma consistente, as maiores taxas de conclusão em todos os níveis de ensino;
- O Nordeste permanece abaixo da média nacional, apesar de avanços graduais ao longo do período analisado;
- A maior taxa de conclusão do ensino superior no Nordeste (15,3%) ainda é inferior à menor taxa registrada no Sudeste;
- Pessoas brancas residentes no Sudeste concentram as maiores taxas de conclusão nos três níveis educacionais analisados;
- À medida que o nível de ensino avança, as desigualdades regionais e raciais se intensificam.

---

## 📈 Visualizações

### Gráfico 1 – Desempenho Regional vs. Média Nacional  
**Taxa média de conclusão do ensino (2012–2022)**

- Sudeste, Sul e Centro-Oeste apresentam desempenho consistentemente acima da média nacional;
- Norte e Nordeste concentram os maiores desafios educacionais.

<img width="899" height="436" alt="image" src="https://github.com/user-attachments/assets/7581a028-74cc-4cf7-8f90-a95cc34cda2b" />

---

### Gráfico 2 – Desempenho por Nível de Ensino

- As disparidades tornam-se progressivamente mais acentuadas do ensino fundamental ao superior.

<img width="418" height="353" alt="image" src="https://github.com/user-attachments/assets/e8b2ce0e-086a-48f9-82e1-11672034aaac" />

<img width="420" height="355" alt="image" src="https://github.com/user-attachments/assets/de248d93-da7e-4b69-b48f-ed3cb183a36b" />

<img width="421" height="358" alt="image" src="https://github.com/user-attachments/assets/a3a77407-da15-41e4-a021-0914a676d461" />

---

## 🏛️ Congresso e Publicação

O artigo resultante deste estudo foi aprovado no **Congresso Nacional Educação, Inovação e Tecnologias** e será apresentado online em **15 de dezembro de 2025**.

O repositório inclui o comprovante de aprovação e inscrição do projeto.

---

## 🧪 Metodologia

As análises foram realizadas com:

- Python  
- pandas  
- plotly  

### Visualização

- HTML  
- CSS  

### Processo aplicado

- Consolidação e limpeza das bases  
- Verificação e validação dos dados  
- Construção dos gráficos e indicadores  

---

## ▶️ Como Executar o Projeto

1. Baixe a base de dados:
2. Execute os notebooks:
- `analise_regional_nacional_completa.ipynb`
- `analise_equidade_racial_educacao.ipynb`

3. Visualize o resultado:
- `infografico_educacao.html`  
(Abra no VS Code ou no navegador)

---

## 👥 Autores

- **Larissa Rodrigues Coelho**  
- **Bruno Santos Silva**

